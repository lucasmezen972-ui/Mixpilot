...
public struct SeratoPlaylistImporter: Sendable {
    public init() {}

    public func importRows(
        _ rows: [SeratoLibraryRow],
        ...
        for row in rows {
            ...
            let duration = fields.compactMap(parseDuration).first ?? 210
            let bpm = fields.compactMap(parseBPM).first ?? 100
            ...
            guard let title = metadataFields.first else {
                warnings.append(PlaylistImportWarning(rowIndex: row.index, message: "Titre introuvable"))
                continue
            }
            let artist = metadataFields.dropFirst().first ?? "Artiste inconnu"

            if fields.compactMap(parseBPM).isEmpty {
                warnings.append(PlaylistImportWarning(
                    rowIndex: row.index,
                    message: "BPM non exposé par Serato : valeur provisoire 100 BPM"
                ))
            }
            if fields.compactMap(parseDuration).isEmpty {
                warnings.append(PlaylistImportWarning(
                    rowIndex: row.index,
                    message: "Durée non exposée par Serato : valeur provisoire 3 min 30"
                ))
            }
