            ...
            VStack(alignment: .leading, spacing: 22) {
                ...
                if let checkpoint = model.checkpoint {
                    GroupBox("Dernier état sauvegardé") {
                        Grid(alignment: .leading, horizontalSpacing: 24, verticalSpacing: 9) {
                            row("Projet", checkpoint.projectName)
                            row("État", checkpoint.state.rawValue)
                            row("Morceau", "\(checkpoint.currentTrackIndex + 1)")
                            row("Deck actif", checkpoint.activeDeck.rawValue)
                            row("Transitions terminées", "\(checkpoint.completedTransitionCount)")
                            row("Dernière commande", checkpoint.lastCommand ?? "Aucune")
                            row("Sauvegardé", checkpoint.updatedAt.formatted(date: .abbreviated, time: .standard))
                        }
                        .padding(8)
                    }
                }
                ...
                if let observation = model.observation {
                    GroupBox("État observé") {
                        HStack(spacing: 22) {
                            Label(observation.isRunning ? "Serato lancé" : "Serato absent", systemImage: observation.isRunning ? "checkmark.circle" : "xmark.circle")
                            Label(observation.accessibilityGranted ? "Accessibilité autorisée" : "Accessibilité bloquée", systemImage: observation.accessibilityGranted ? "hand.thumbsup" : "hand.raised.slash")
