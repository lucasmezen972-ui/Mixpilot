...
-- Views are recreated because PostgreSQL cannot replace a view while changing
-- its public column list.
...
alter table public.mixpilot_events add column if not exists expires_at timestamptz;
...
do $$
begin
    if not exists (select 1 from pg_constraint where conname = 'mixpilot_devices_dj_backend_check') then
        alter table public.mixpilot_devices
            add constraint mixpilot_devices_dj_backend_check
            check (dj_backend is null or dj_backend in ('djay','rekordbox','serato'));
    end if;
    if not exists (select 1 from pg_constraint where conname = 'mixpilot_sessions_dj_backend_check') then
        alter table public.mixpilot_sessions
            add constraint mixpilot_sessions_dj_backend_check
            check (dj_backend is null or dj_backend in ('djay','rekordbox','serato'));
    end if;
    if not exists (select 1 from pg_constraint where conname = 'mixpilot_devices_mapping_sha256_check') then
        alter table public.mixpilot_devices
            add constraint mixpilot_devices_mapping_sha256_check
            check (mapping_sha256 is null or mapping_sha256 ~ '^[A-Fa-f0-9]{64}$');
    end if;
    if not exists (select 1 from pg_constraint where conname = 'mixpilot_sessions_mapping_sha256_check') then
        alter table public.mixpilot_sessions
