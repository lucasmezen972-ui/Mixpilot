...
TEXT_SUFFIXES = {
    ...
    ".sql", ".yml", ".yaml", ".json", ".toml", ".plist",
    ".xcprivacy", ".strings", ".md", ".txt", ".xml",
...
TEST_PREFIXES = (
    "Tests/",
    "Mobile/MixPilotRemote/XcodeTests/",
    "Shared/RemoteProtocolV2/Tests/",
)
...
SWIFT_HARD_ERRORS = [
    ...
    ("forced-try", re.compile(r"\btry!\s+"),
...
SHELL_HARD_ERRORS = [
    ("shell-eval", re.compile(r"(^|[;&|]\s*)eval\s+"),
     "eval is forbidden in release/build scripts."),
...
def git_tracked_files() -> list[Path]:
    try:
        ...
        values = [item for item in output.decode("utf-8").split("\0") if item]
...
def scan_common(path: str, text: str, findings: list[Finding]) -> None:
    ...
    for name, pattern in SECRET_PATTERNS:
        for match in pattern.finditer(text):
            add(findings, "error", "embedded-secret", path,
                line_number(text, match.start()), f"{name} appears embedded in the repository.", match.group(0))

    for match in ABSOLUTE_USER_PATH.finditer(text):
        add(findings, "error", "personal-absolute-path", path,
...
def scan_swift(path: str, text: str, findings: list[Finding]) -> None:
    ...
    for index, line in enumerate(lines):
        if "@unchecked Sendable" in line or "nonisolated(unsafe)" in line:
            if not has_safety_comment(lines, index):
                add(findings, "error", "unsafe-concurrency-without-proof", path, index + 1,
                    "Unsafe concurrency opt-out requires a nearby SAFETY comment proving immutability/thread safety.",
                    line)
...
def scan_shell(path: str, text: str, findings: list[Finding]) -> None:
    if path.startswith("Scripts/") and not text.startswith("#!"):
        add(findings, "error", "missing-shebang", path, 1,
            "Executable script is missing a shebang.", text.splitlines()[0] if text else "")
    if path.startswith("Scripts/") and "set -euo pipefail" not in text:
        add(findings, "error", "weak-shell-options", path, 1,
            "Release/support scripts must enable set -euo pipefail.", "")
...
def scan_sql(path: str, text: str, findings: list[Finding]) -> None:
    ...
    for match in re.finditer(r"\bsecurity\s+definer\b", lower):
        window = lower[match.start():match.start() + 1200]
        if "set search_path" not in window:
            add(findings, "error", "security-definer-search-path", path, line_number(text, match.start()),
...
def scan_repository_invariants(paths: list[Path], findings: list[Finding]) -> None:
    ...
    for rel in rels:
        ...
        if parts & GENERATED_PATH_PARTS or rel.endswith((".xcuserstate", ".DS_Store")):
            add(findings, "error", "tracked-generated-file", rel, 1,
                "Generated/user-specific file is tracked.", rel)
    ...
    for rel in rels:
        if rel.startswith("supabase/migrations/") and rel.endswith(".sql"):
            ...
            if previous:
                add(findings, "error", "duplicate-migration-prefix", rel, 1,
                    f"Migration timestamp collides with {previous}.", prefix)
    ...
    if "Mobile/MixPilotRemote/Sources/RemoteConnection.swift" in swift_files:
        ...
        if not info.exists():
            add(findings, "error", "missing-ios-info-plist", "Mobile/MixPilotRemote", 1,
                "iOS Remote has no Info.plist.", "")
        else:
            try:
                payload = plistlib.loads(info.read_bytes())
                if not payload.get("NSLocalNetworkUsageDescription"):
                    add(findings, "error", "missing-local-network-purpose", relative(info), 1,
                        "Bonjour app needs NSLocalNetworkUsageDescription.", "")
                if "_mixpilot._tcp" not in payload.get("NSBonjourServices", []):
                    add(findings, "error", "missing-bonjour-service", relative(info), 1,
                        "Info.plist does not register _mixpilot._tcp.", "")
            except Exception:
                pass

    uses_user_defaults_mobile = False
    for rel in swift_files:
        if not rel.startswith("Mobile/MixPilotRemote/"):
            continue
        text = read_text(ROOT / rel) or ""
        if "UserDefaults" in text or "@AppStorage" in text:
            uses_user_defaults_mobile = True
            break
    ...
    if privacy.exists():
        try:
            ...
            if uses_user_defaults_mobile and "CA92.1" not in reasons.get(
    ...
    if project_yml.exists():
        ...
        if "SWIFT_VERSION: 6.0" not in text:
            add(findings, "error", "ios-swift-version", relative(project_yml), 1,
                "iOS target must compile in Swift 6 mode.", "")
        if "ENABLE_USER_SCRIPT_SANDBOXING: YES" not in text:
            add(findings, "error", "ios-script-sandbox", relative(project_yml), 1,
                "User script sandboxing must stay enabled.", "")
...
def write_reports(findings: list[Finding], files: int, lines: int, output_dir: Path) -> None:
    ...
    (output_dir / "ultimate-audit.json").write_text(
    ...
    for item in ordered:
        rows.extend([
            ...
            f"- {item.message}",
            f"- Excerpt: `{item.excerpt.replace('`', chr(39))}`" if item.excerpt else "",
            "",
        ])
...
def main() -> int:
    ...
    for path in tracked:
        ...
        if text is None:
            if path.suffix.lower() in TEXT_SUFFIXES:
                add(findings, "error", "unreadable-text-file", rel, 1,
                    "Expected text file is binary or not UTF-8.", "")
            continue
        files_scanned += 1
        ...
        if suffix == ".swift":
            scan_swift(rel, text, findings)
        elif suffix in {".sh", ".bash", ".zsh"}:
            scan_shell(rel, text, findings)
        elif suffix == ".sql":
            scan_sql(rel, text, findings)
